...
Дата: 2026-08-15. Статус: утверждён (закрывает impresario#14, блокер
...
Формально: **решение активно, если на него не ссылается `supersedes`
никакое другое schema-valid решение того же bundle**. Следствия:

- Самоссылка (`supersedes` на собственный `decision_id`) и циклы
  супер-цепочки — нарушение `LRD_SUPERSEDES`: формально самоперекрытие
  прошло бы identity-проверку и ошибочно убрало бы единственное решение
  из active-set.
...
Порядок и атомарные границы — нормативные, для любого бэкенда:

1. Прочитать и провалидировать `loop.state` (fail-closed: невалидный
   state — типизированный отказ).
2. CAS-предусловие: `stop.verdict = needs_human`; identity ожидания —
   `(loop_id, stop.iteration)`. `failed` не resumable.
3. Найти активный LRD этой identity; если его нет —
   validate-then-atomic-write создать новый.
4. Повторно проверить CAS непосредственно перед потреблением (без этого
   «subject совпал» оставляет TOCTOU-окно между проверкой и записью
   состояния).
...
Любой провал — отказ, ожидание сохраняется (fail-closed).
...
- Протокольные тесты resume: happy path (LRD записан и потреблён, trace
  ...
  fail-closed, ожидание сохраняется; TOCTOU-повторный CAS.
