...
завершающей итерацию стадии creator. Как и `resume_loop` и ingest
оценщика, весь протокол шага (check-and-write целиком) идёт под
single-writer lock workspace — конкурентный писатель на том же
workspace отказывает быстро, а не гонится с этим шагом за freshness.

Раннер может продвинуться до терминального вердикта прямо из уже
персистентных RP+CD (workspace, ранее остановленный на `apply:N` /
`evaluate:N`, или крэшнувшийся в этом окне), ни разу не спросив
`SingleAnswerAgent` — тогда собранный `step`-артефакт остаётся
непотреблённым и на диск не попадает. `step` не сообщает о
материализации, которой не было: он проверяет по факту (тем же
поиском по `provenance.brief_id`, что и идемпотентность в шаге 2), был
ли артефакт персистирован, и при отрицательном исходе возвращает
`{"artifact": null, "noop": true, "runner": {...вердикт...}}` — форма,
которую невозможно спутать с материализованным артефактом. `ok`
отчёта — `verdict != "failed"` (как и exit code `forconcept run`):
терминальный `failed` не превращается в успех ни в отчёте, ни в CLI
exit code.
...
- **`waiting_since` ожидания := `stop.at`** (impresario#40). `stop.at` —
  `now`-вход запуска, материализовавшего остановку: пишется один раз,
  атомарно, вместе с самим переходом и не освежается повторными
  запусками (re-run — no-op). Время — вход детерминированного раннера,
  не wall-clock записи: переход случился не раньше `at` и не позже
  появления файла, так что это доказуемая **не-поздняя граница** начала
  ожидания. Возраст ожидания и decision latency (ADR-ECO-009 §8),
  посчитанные от `stop.at`, — верхняя оценка: завышение ограничено
  длительностью материализовавшего запуска (в пошаговом step-флоу,
  где артефакты итерации предсуществуют, — секунды). Выводить
  `waiting_since` из mtime/`updatedAt` файла нельзя; при отсутствующем
  или невалидном `stop` возраст ожидания — `unknown`, не суррогат.
